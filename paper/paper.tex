...
Io has been observed during occultations by Jupiter and other Galilean moons using high-cadence near infrared photometry since the 1980s. 
These observations encode a wealth of information about its surface.
They are ideal for testing methods for inferring surface maps of occulted bodies from time series observations.
    We built a generative model of Io's surface using the code \textsf{starry} \href{https://rodluger.github.io/starry/}{\color{linkcolor}\faGithub} which enables fast, analytic, and differentiable computation of occultation light curves in emitted and reflected light.
    Our probabilistic Bayesian model is able to recover known bright features on the surface of Io using only two light curves and without any assumptions on the locations, shapes or the number of these features.
    The methods we have developed are also easily applicable to the problem of mapping stars and exoplanets. \href{https://github.com/fbartolic/volcano}{\color{linkcolor}\faGithub}
\end{abstract}
...
Besides being useful for modeling secondary eclipses of volcanic Super-Earths, our model is also likely to be useful for modeling features on gaseous exoplanets with sparse surface features.
