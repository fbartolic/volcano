...
\affil{Department~of~Geology~\&~Geophysics, University~of~Wyoming, Laramie, WY}
%
...
Assuming we have a single light curve with $T$ data points and a map set by the pixels $\mathbf{p}$, the (Gaussian) log likelihood is given by
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:flux_model}
\end{equation}
where $\mathbf{A}'\equiv\mathbf{A}\mathbf{P}^\dagger$ and $b$ is a constant flux offset parameter which we have added to account for stray flux which cannot be attributed to Io and is likely due to residual light from Jupiter.
...
\begin{figure}[t!]
    ...
    \oscaption{pixels_vs_harmonics}{%
       ...
       Each of the columns beneath the true map shows the (median) posterior estimate of the map (top), the data and the posterior flux samples (middle row) and the residuals (bottom row).
        ...
        With the pixel model we are able to recover the true map precisely while the spherical harmonic model fails to do that. 
...
With this prior, the model is able to accurately recover the true map.
        The plot shows the true $l=20$ map (top), the posterior (median) estimate of the inferred map (second row), the inferred map as seen by the observer during the occultation (small circles), the data and posterior samples of the flux (orange lines), and the normalized residuals (bottom).
...
\begin{figure}[t!]
    ...
    \oscaption{irtf_main_plots}{%
       ...
       \label{fig:irtf_1998_errorbarss}
    }
    \end{centering}
\end{figure}
