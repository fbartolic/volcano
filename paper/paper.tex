...
\begin{figure}[h!]
    ...
    \oscaption{schmixels_to_pixels}{%
        ...
        so to differentiate between pixels mapped to spherical harmonics and pixels constructed from spherical harmonics we call the former "schmixels".
        Since there are many more schmixels than there are spherical harmonics coefficients 
...
\begin{figure}[h!]
    ...
    \oscaption{schmixels_vs_harmonics}{%
        ...
        The map in the middle comes from a model in which we instead fit for schmixels $\mathbf{p}'$ with a Half Gaussian prior but still use $\mathbf{y}$ to compute the flux.
        ...
        While all models fit the data reasonably well, the ones in which we fit for schmixels result in more plausible and sparse maps. The location of the true spot is not recovered correctly because a single light curve does not contain enough information to break the degeneracy in position along the limb of the occultor.
        \label{fig:schmixels_vs_harmonics}
    }
    \end{centering}
\end{figure}
...
The schmixel model with Half Gaussian priors produces a more sensible map in which the unobserved hemisphere has much lower intensity but the arc is less sharply defined and more extended and as a result the predicted flux does not rise steeply enough compared to the light curve.
The map from a model with an Exponential prior on the schmixels provides the best fit.
It is the sparsest of the three maps, having a single bright spot at the center of the projected limb of Jupiter at the location of the true spot with extended tails along the limb.
In principle, the sparsity is a consequence of the Exponential prior which has significant probability mass near zero, meaning that it pushes most schmixels towards zero intensity, and a heavy tail towards large intensities.
It favors parse maps with near zero intensity everywhere except for a few high intensity regions which is exactly the kind of prior information we want to assume for Io because we expect spot like features on its surface.
In practice, as shown in Figure~\ref{fig:schmixels_to_pixels}, the Exponential prior 
implies a more Gaussian like prior in spherical harmonic space and yet it still gives noticeably different looking map than a Gaussian (NOT CONVINCING). 

%Figure~\ref{fig:pixels_hist} shows the cumulative distributions of pixels and schmixels corresponding to the three inferred maps shown in Figure~\ref{fig:schmixels_vs_harmonics}, shown in the same order as the inferred maps.
%The plot on the left shows the distribution of pixels in the true map (blue line) 
%and the inferred pixels $\mathbf{P}\,\mathbf{y}$ (orange).
%The true distribution rapidly increases towards unity because the majority of pixels have near zero intensity. 
%There is a very small tail towards negative values which is due to the fact that we generate the true map at $l=30$ and even at that resolution there is still some ringing present leading to negative intensities.
%The inferred distribution has a very different shape with approximately 20\% of pixels having negative values and a much more gradual increase towards unity.
%
%The second subplot shows the distribution of both pixels (orange) and schmixels (green) corresponding to the model with Half Gaussian priors on the schmixels.
%The distribution of pixels and schmixels in this case is nearly identical meaning that the approximate mapping $\mathbf{P}^\dagger$ is good enough.
%In the case of an Exponential prior on schmixels (right) the shape of the schmixel and pixel distributions is similar to the Gaussian prior solution except for a small discrepancy between the two distributions.
%The schmixel distribution has no mass at negative values (by definition), but the inferred pixel distribution has a very small negative tail.
%The discrepancy is likely due to the fact that the Exponential prior favors more concentrated regions of high intensity which induce more ringing artefacts with some negative pixels.
%
%\begin{figure}[h!]
%    \begin{centering}
%        \includegraphics[width=\linewidth]{figures/pixels_hist.pdf}
%    \oscaption{pixels_hist}{%
%        Cumulative distribution of pixels in the true map and the and schmixels/pixels of inferred maps shown in Fig.~\ref{fig:schmixels_vs_harmonics}.
%        The distribution of pixels in the inferred map from the $Y_{lm}$ model (left) is very different from the truth and it has a significant left tail towards negative values.
%        On the other hand, the distribution of pixels in the schmixel model (middle and right) is much closer to the truth and it has a negligible fraction of negative intensities.
%        The distribution of pixels closely follows the distribution of schmixels for both choices of priors on schmixels. 
...
To suppress ringing which results in regions of negative intensity surrounding a spot like feature as in Figure~\ref{fig:schmixels_vs_harmonics}, we choose to apply a spatial smoothing filter to the spherical harmonics.
...
In Figure~\ref{fig:smoothing_kernel} we show the effect of (Gaussian) smoothing on a spherical harmonic expansion of a spot with a Gaussian intensity profile.
The expansion is in the quantity $\cos\Delta\theta$ where $\Delta\theta$ is the angular separation between the center of the spot and another point on the surface of the sphere.
...
The leftmost subplot shows the expansion with no smoothing ($\sigma_s=0$) in which case the symmetric ringing around the center of the spot is clearly visible even at relatively high order ($l=20$).
...
In this case the negative ringing is a lot less visible albeit at the cost of having a slightly larger spot because suppressing higher order harmonics necessarily means that we lose some ability to represent smaller scale features.
This is most apparent in the rightmost subplot where $\sigma_s=0.2$. 
Although there is practically no ringing, expansions at $l=10$, $l=15$ and $l=20$ look almost the same because all coefficients above $l=5$ are significantly suppressed.
...
Thus, there is a trade-off between smoothing and resolution of the maps. 
...
In Figure~\ref{fig:ingress_egress_simulated} we test a model with the smoothing filter 
by simulating data as in Figure~\ref{fig:schmixels_vs_harmonics}, except this time we simulate two light curves, an ingress light curve generated from real ephemeris of Io disappearing behind the limb of Jupiter and egress light curve generated when Io reappears.
...
\subsection{Fitting IRTF ingress/egress light curves}
\begin{figure}[h!]
    \begin{centering}
    \includegraphics[width=1.\linewidth]{figures/irtf_ingress_egress.pdf}
    \oscaption{irtf_ingress_egress}{%
Maximum A Posteriori (MAP) estimate of two $l=25$ maps produced by simultaneously fitting two IRTF light curves of an occultation of Io by Jupiter.
The light curve on the left corresponds to an ingress of Io 
        while the one on the right
        fitting a pair of light curves simulated from the true map which is shown on the left.
        The simulated light curves are shown in the third panel from the top, an ingress light curve on the left and an egress light curve on the right.
        The small circles above each of the light curves show the progression of the occultation over the inferred map.
        The precise location of the true spot is not recovered for either of the two models
        which is not surprising because a single light curve is not sufficient to break the degeneracies.
