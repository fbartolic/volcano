...
\begin{figure}[ht!]
    ...
    Median estimates of two $l=20$ maps (top panel) inferred by fitting a pair of observations of occultations of Io by Jupiter from 1998.  
        The map and light curve correspond to the ingress event and the ones on the right are the egress event. 
        The two maps are (by construction) the same except for a difference in the overall amplitude. 
        The observations were taken using the IRTF telescope in the NIR filter (TODO: which filter).
The small circles beneath the inferred map show the progression of the occultation. 
        Orange lines show posterior samples from the physical model (excluding the Gaussian Process).
        The residuals (bottom) are computed with respect the median prediction from the physical model and the Gaussian Process predictions are shown on the same plot.
The inferred map shows two hotspots, the bright hot spot is emission due to Loki and the faint hot spot is Kanehekili.
...
\begin{tabular}{l l l}
 ...
 $\tau$ &  & \\
    ...
    $\rho_{\mathrm{GP},E}$ &&  
\end{tabular}
    \caption{Inferred model parameters for the pair of occultations from 1998.}
    \label{tab:1998_parameters}
\end{table}
...
\begin{figure}[ht!]
    ...
    \oscaption{irtf_ingress_egress_loki}{%
        Zoom in on the bright hot spot from Figure~\ref{fig:irtf_ingress_egress_1998} overlaid on top of the U.S. Geological Survey's map of the surface of Io (CITE) which was        constructed from Galileo observations.
