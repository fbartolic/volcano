...
Maximum A Posteriori (MAP) estimate of an $l=25$ map (right) produced by fitting a pair of simulated ingress and egress occultation light curves (bottom) which were generated from a $l=25$ map consisting of a single bright spot (left). 
       ...
       Small circles beneath each inferred map show the progression of the occultation.
        The model is able to recover the position of the true spot (marked with a grey cross) and there is noticeably less ringing than in Figure~\ref{fig:schmixels_vs_harmonics}.
        \label{fig:ingress_egress_simulated}
    }
    \end{centering}
\end{figure}

The inferred map looks significantly better than the one in Figure~\ref{fig:schmixels_vs_harmonics}. 
...
In the next section, we fit a pair of light curves from the IRTF dataset described in Section~\ref{sec:data}.
