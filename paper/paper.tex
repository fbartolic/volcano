...
These observations encode a wealth of information about the volcanic features on its surface.
    ...
    Using this model we are able to recover surface emission maps of Io containing known volcanic hotspots, without having to make assumptions about the locations, shapes or the number of these hotspots.
...
Since the IRTF light curves are not provided with estimated errorbars, we choose to fit for all errorbars simultaneously using a hierarchical approach in which we assign a global scale $l$ for all errorbars in a given light curve and we draw each individual errobar from a Half Normal distribution with standard deviation equal to that scale.
That is, we have 
\begin{align}
    \sigma_{i,I}&\sim \mathcal{N}^+(0, l_E),\quad i=1,\dots,N_I\\
    \sigma_{i,E}&\sim \mathcal{N}^+(0,l_I),\quad i=1,\dots, N_E
\end{align}
where $\sigma_{i,I}$ are the errorbars for the ingress light curve and $\sigma_{i,E}$ are the errorbars for the egress light curve.
The symbol $\mathcal{N}^+$ denotes a truncated Gaussian distribution restricted to positive values.
Treating errorbars as free parameters means that we are introducing hundreds of more parameters (as many as there are data points) which is not an issue as long as these parameters are constrained by the data and our model is regularized.
The noise model as defined above is extremely flexible, it can account for a given feature in the light curve by inflating individual errorbars (white noise) or by varying the characteristic timescale $\rho_\mathrm{GP}$ in the Mat\'ern 3/2 kernel.
...
The posterior distribution for the errorbars is shown in Figure~\ref{fig:irtf_1998_errorbars}.
Both the global scale for of the errorbars and the individual errorbars are well constrained by the data. 
...
We also fit for all errorbars simultaneously using a hierarchical model and we plot the median estimates of those errorbars.
...
%        The errorbars are estimated using a hierarchical model in which we assume  that each errobar is drawn from a Half Gaussian distribution with a global scale parameter common to all errorbars in a single light curve.
...
The model inflates the errorbars around those times because it unable to make the spot small enough.
...
\begin{table}[t!]
    \begin{center}
        \begin{longtable}{W{l}{2cm} W{l}{5cm} W{l}{3cm}  W{l}{3cm}}
            ...
            \\
            $l_I$ & errobar scale ingress &$0.97_{-0.06}^{+0.06}$ & GW/um/sr
            \\
